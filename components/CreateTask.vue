    ...
    <div>
      ...
      <div v-if="toggleCreateTask.showMenu" class="fixed h-3/5 bottom-14 left-0 right-0 bg-white p-4 shadow-lg">
        <h3 class="text-lg font-semibold">New Task</h3>
          ...
          <div v-else>
            <button @click="updateTaskStore.updateTaskItem(toggleCreateTask.taskId, !useTaskStore.msg ? readTask.fetchedData : useTaskStore.msg, useTaskStore.dueDate, useTaskStore.time)" class="mt-4 bg-blue-500 text-white px-4 py-2 rounded-md shadow-lg hover:bg-blue-600">
              Update Task
            </button>
          </div>
